const fs = require('fs');
const jsonData = fs.readFileSync('result 1.json', 'utf8');
...
const resultArray = Object.entries(intermediateResult).map(([word, count]) => ({ word, count }));
resultArray.sort((a, b) => b.count - a.count);

const resultJSON = resultArray.map(item => JSON.stringify(item)).join(',\n');
const finalJSON = `[\n${resultJSON}\n]`;
fs.writeFileSync('result.json', finalJSON, 'utf8');
// 提取需要分析的数据字段
